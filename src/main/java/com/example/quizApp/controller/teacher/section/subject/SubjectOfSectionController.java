...
public class SubjectOfSectionController {
	...
	ResponseEntity<SubjectResult.Add> addSubject(@PathVariable String sectionName, @RequestBody @Valid AddSubjectRequest req) {
		
		var result = subjectService.addSubject(sectionName, req);
		
		return switch (result) {
		case SUBJECT_ALREADY_EXISTS -> ResponseEntity.status(409).build();
